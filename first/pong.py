...
class Ball():
    ...
    def reflect(self, offcenter, direction, player):
        ...
        if player == 1:
            self.vector = (3 * (math.cos(math.radians(bounce_angle)) + 1), 8 * -math.sin(math.radians(bounce_angle)))
        else:
            self.vector = (-3 * (math.cos(math.radians(bounce_angle)) + 1), 8 * math.sin(math.radians(bounce_angle)))
...
class Pong():
    ...
    def step(self, actions):
        ...
        next thing is to fix that it runs multiple episodes, reset properly
